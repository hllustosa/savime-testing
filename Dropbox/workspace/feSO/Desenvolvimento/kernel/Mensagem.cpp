...
//declaração da instância global da classe entregador
volatile Entregador entregador;

//função para notificar o entregador das IRQS
...
void Entregador::inicializar(Irq irq)
{
   ...
   //instalando função para notificar IRQ
   for(int  i = 0; i <= 15; i++)
   {
...
//Método que serve para notificar o entregador em caso de IRQ
...
{	
    ...
    if(&p != NULL)    
	{
		//Criando Mensagem com conteúdo IRQ
...
//Método que serve para notificar o entregador em caso de evento do kernel
void Entregador::notificar_evento(char * dados, int evt_num)
{
	...
	//Removendo Listerners, caso seja necessário
...
//Metódo para acessar os dados de uma porta
Porta& Entregador::obter_porta(int num)
{
	return portas[num];
}

//Método para adicionar listerner de enveto do kernel
int Entregador::adicionar_listerner(unsigned pid, unsigned porta)
{
	//Verificando se não é uma porta utilizada pelo kernel
	if( (porta < KERNEL_PORTA_LIM_INF) || (porta > KERNEL_PORTA_LIM_SUP) )
	{
		Porta& _p = portas[porta];
		
		//caso já exista algum processo escutando a porta
		if(&_p != NULL)
		{
...
//envio de mensagem assíncrona para um pid
...
//Envio de Mensagem Síncrona para uma porta
int Entregador::tratar_envio(struct regs *r, unsigned pid, unsigned porta, int sinalizar ,unsigned char msg[TAM_MAX_MSG])
{
	...
	//Verificando se a porta já foi alocada
	if(&p != NULL)
	{   
		...
		{
			//se a mensagem não for originária do KERNEL	
			if(pid != KERNEL_MSG)
			{
...
//Recebimento de Mensagem Síncrona em uma porta
int Entregador::tratar_recebimento(struct regs *r, unsigned pid, unsigned char * msg, int porta)
{
	...
	//Se a referência for nula, não há mensagens
	if(&m != NULL)
	{		
		...
		//copiando mensagem para espaço de endereçameno do processo
...
//Obtém lista de Portas e seus respectivos processos responsáveis
