...
//Instância glocal do vfs
volatile Vfs vfs;

//Adiciona arquivo a um diretório
int Diretorio::adicionar_arquivo(unsigned int desc)
{
	//se não houver alcançado número máximo de arquivos
...
//Remove arquivo de um diretório
...
//Carrega arquivo em um bloco contínuo de memória
...
{		
	...
	{
		//caso o diretório pai seja um diretório montado
		if(arq.tipo == Vfs::MNT_DIR)
		{
			//obter porta do processo responsável
			Porta &p = entregador.obter_porta(arq.porta_servidor);
			
			/*
			  Se o pid responsável for diferente do
			  pid em execução, significa que outro processo está
		...
		//atribuíndo descritor ao arquivo
...
//Sobrecargas para o método de abertura de arquivo
...
unsigned int Vfs::_abrir(unsigned char * nome, char modo, unsigned int * novo_descritor, unsigned int * servidor)
{
	...
	*novo_descritor = 0;
	...
	while(trecho != NULL)
	{
	   ...
	   {
			//obtendo lista de arquivos em diretorio (máximo 127)
			Diretorio * dir = (Diretorio *)arq.clusters[0].dados;	
			
			//varrendo lista de diretórios
	   ...
	   else
	   {
		  ...
		  else //senão, o diretório não foi encontrado
		  {
	...
	if(!encontrou)
	{
	     ...
	     *novo_descritor = 0; 
		 ...
		 //'d' = verificar se diretório existe
		 if(modo == 'd')
	     {	
			return VFS_DIR_NAO_ENCONTRADO;
	     }
	...
	{	 
		...
		if(arq.aberto_por == 0)
		{
		   ...
		   //retorna descritor caso diretório exista
		   else if(modo == 'd')
	       {
			  if( (arq.tipo != Vfs::MNT) 
...
unsigned int Vfs::escrever(unsigned int descritor, unsigned char * dados, unsigned int tamanho)
{
	...
	if(&arq != NULL)
	{		
		//verificando se o arquivo está aberto e não é um diretório
		if( (arq.aberto_por == escalonador.obter_pid_em_execucao()) && (arq.tipo != Vfs::DIR))
		{
			...
			while(bytes_a_escrever > 0)
			{
				...
				//caso a quantidade de bytes a escrever não caiba no bloco atual
				...
				{
					//copiar tudo que for possível
					qtd_copiar = bytes_a_escrever;
				} 
				
				
				//diminuir a quantidade de bytes a escrever de acordo com o que será copiado
				...
				//obtendo último bloco do arquivo
	...
	{
		//Retornando erro de arquivo não encontrado
		return VFS_ERR_ARQ_NAO_ENCONTRADO;
	}
}
...
//Configurar posição dentro do arquivo no qual a próxima leitura/escrita deve ocorrer
unsigned int Vfs::buscar(unsigned int descritor, unsigned int pos)
{
	...
	if(&a != NULL)
	{
		//verificando se a posição escolhida está dentro dos limites do arquivo
		if(pos > a.tamanho)
		{
			return VFS_POSICAO_INVALIDA;
		}
		//verificando se o processo em execução está com o arquivo aberto
		...
		//se a posição for menor que o arquivo e ele estiver aberto é possível alterar o offset
		else
		{
			a.down();
			a.offset = pos;
...
//Adicionar nó para um arquivo em um sistema de Arquivos Externo
unsigned int Vfs::adicionar_no(unsigned char * nome , int tamanho, unsigned int porta_servidor, unsigned int * descritor)
{
	...
	if(tamanho == -1)
	{
		modo = 'D'; //caso o tamanho seja -1, criar um diretório
		tipo = Vfs::MNT_DIR;
	}
	else
	{
		modo = 'a'; //senão, criar um arquivo 
		tipo = Vfs::MNT;
	}
...
//Torna nó externo acessível dentro do VFS
...
//Torna nó externo inacessível dentro do VFS
...
unsigned int Vfs::obter_info_arquivo(unsigned descritor, unsigned char * dados)
{
	...
	if(&a != NULL)
	{
	   ...
	   //copiando para dados	
	   memcpy(dados, (unsigned char *)&info, sizeof(Arq_Info));
