...
//Função para a Syscall externas, definidas em start.asm
extern "C" void systemcall();

//Instância da classe syscall
Syscall syscall;
...
void Syscall::inicializar(Idt idt)
{
    ...
    //adicionando o endereço da função systemcall como o 80ª entrada na IDT
    idt.adicionar_registro(80, (unsigned)systemcall,  0x08, 0xEE);
}
 
//Função que executa a chamanda ao sistema referente ao código no registrador EAX
...
void Syscall::sbrk(struct regs *r)
{
	...
	//aumentando a quantidade de memória solicitada	 
    p.aumentar_heap(&end, r->ebx);

	//colocando novo endereço em eax
	r->eax =  end;

}

//Obter informações sobre o Kernel
...
void Syscall::enviar_msg(struct regs *r)
{
	...
	//verificando se o o ponteiro está dentro do espaço de endereços do programa
...
void Syscall::escutar_porta(struct regs *r)
{
   ...
   //verificando se o processo é um DRIVER, ou se a porta é maior que 1000
   //as 1000 primeiras portas são reservadas para as IRQS e drivers
...
void Syscall::abrir_arquivo(struct regs *r)
{
	...
	//colocando o descritor do arquivo em ebx, e o retorno da função em eax
	r->eax = ret;
...
void Syscall::ler_arquivo(struct regs *r)
{
	...
	{
		//chamando função ler do sistema de arquivos virtual
...
{
	//apenas drivers podem executar essa função
...
void Syscall::executar(struct regs *r)
{
	...
	//alocando memória para o array de parâmetros
	unsigned char ** cargs = (unsigned char **)kmalloc( sizeof(unsigned char *)*10);	
	
	//separando os parametros divididos por espaços em branco
	char * tok = strtok(args," ");
   
    //quando tok é nulo, final da string alcançado
	while (tok != NULL)
    {
	   //tamanho do token	
	   int tam_param = strlen(tok)+1;
	   
	   //alocando memória para mais um parâmetros
	   cargs[argc] = (char *)kmalloc(tam_param);

	   //copiando da string para o array de parâmetros	
       memcpy(cargs[argc], tok, tam_param);	
	   
	   //obtendo próximo token
	   tok = strtok(NULL," ");
		
	   //incrementando contagem de parâmetros	
	   argc++;
    }
	...
	int res = escalonador.adicionar_processo(nome, imagem, Processo::USUARIO, argc, (char **)cargs);
	 ...
	 //liberando memoria ocupada pelos parâmetros
...
// Função chamada a partir de start.asm em resposta a uma systemcall
